...
async function getTrendsData(days: number) {
  ...
  // Group by date, compute weighted fleet averages
  ...
  return Array.from(byDate.entries())
    ...
    .map(([date, { sumDownload, sumUpload, totalTests }]) => ({
      ...
      avg_download:
        totalTests > 0
          ? Math.round((sumDownload / totalTests) * 100) / 100
          : 0,
      avg_upload:
        totalTests > 0
          ? Math.round((sumUpload / totalTests) * 100) / 100
          : 0,
    }))
}
...
export default async function TrendsPage({
  ...
  const [trendsData, timeOfDayData] = await Promise.all([
    getTrendsData(days),
