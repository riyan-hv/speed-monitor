...
export default function AlertsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      ...
      <p className="text-sm font-semibold text-gray-700 mb-1">Failed to load alerts</p>
      <p className="text-xs text-gray-400 mb-4 max-w-xs">{error.message || 'Could not fetch alert configs from Supabase.'}</p>
